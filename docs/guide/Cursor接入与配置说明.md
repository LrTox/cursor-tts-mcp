...
在旧对话里：子代理结束 / Task 工具后会注入「请调用 speak」提醒（非全文朗读）。详见 [Hook调研结论](../plan/Hook调研结论.md)。
...
|------|----------|
| 实质进展 | `progress` |
| **任务结果 / 结论交付**（必说） | `summary` 或 `progress` |
| 排查结论 | `progress` |
...
纯读文件/小检索/琐碎编辑；朗读代码/日志/长文；工具调用成功本身。

也可粘贴到 Cursor User Rules：

```text
【语音播报 cursor-tts】
1. 实质进展、报错阻塞、需要用户决策/操作时，立即 speak 一句短中文。
2. 向用户交付任务结果/结论/操作指引时，必须再 speak 一句（category=summary），禁止只打字不说话。
3. 不要等整任务结束才第一次说话；中间进展与最终结果都要说。
4. 不要朗读代码、日志、长文；纯中间细节不要说。
5. 参数：进展 progress；报错 error+interrupt；决策 decision；结果 summary。
6. 用户要安静时调用 stop。禁止全文朗读。
```

### 5.1 反例（禁止）

- 只打字给结论，不调用 `speak`
- 只在最后说「任务完成了」，中间重要进展全无
- `speak` 传入整段分析报告
- 每读一个文件就播报一次
